...
nematode %>% 
  mutate(rndvi = (nir - r)/(nir +r)) %>% 
  mutate(bndvi = (nir - r)/(nir +r)) %>%
  mutate(gndvi = (nir - r)/(nir +r)) 
```
...
we need to wrap arguments with flower brackets in the body of function.
for assignment we need to use those flower brackets (wrap argument names in the body of function)
...
for (i in 3: ncol(nematode_w)) {
  ...
  # if we use [3] we get the whole column but with [[i]] we get the vector. so we use double beackets instead of single bracket
...
nematode_mods_sel <- nematode_mods %>% 
  ...
  arrange(r2) %>% 
  group_by(resp_var) %>% 
#subset the data(one best for each nematode)
 filter(r2 == max(r2))
...
nematode_mods_sel$data[1]

nematode_mods_sel_plots$splot[7]
